...
transaction_years = [2018, 2019, 2020, 2021, 2022]  # Replace with actual transaction years
...
if 'df_top_transactions' in locals():
    if not df_top_transactions.empty:
        st.subheader('Top 3 Transactions for {} in Q{} {}:'.format(state_name, quarter, transaction_year))
        st.write(df_top_transactions)
    else:
        st.write('No transactions found for selected parameters.')
